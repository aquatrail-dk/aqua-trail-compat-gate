...
function doGet() {
  return HtmlService
    ...
    .setTitle('Aqua Trail — HTMLService Gate');
}
...
function api(req) {
  try {
    ...
    const session = requireSession_(String(req.token || ''));

    if (action === 'events') {
      return {
        ok: true,
        data: { events: listEvents_(session) }
      };
    }

    if (action === 'event_bundle') {
      const eventId = String(req.event_id || '');
      return {
        ok: true,
        data: {
    ...
    if (action === 'logout') {
      CacheService
        .getScriptCache()
        .remove('session:' + String(req.token || ''));
